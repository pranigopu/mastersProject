...
Consider the basic random process of a coin toss. Here, we do not specify the coin as fair, instead giving it a $\theta$ probability of getting heads. Let us focus on getting heads, which means we define the probability of getting heads as the probability of success. Now, consider there to be a fixed number of trials $n$. Now, consider the observed data $D$ wherein we get $k$ heads in $n$ trials. Here is where we use the binomial distribution to define the likelihood of the observed data being generated by the random process with the probability of success being $\theta$:

$\mathbb{P}(D | \theta) = {n \choose k} $
